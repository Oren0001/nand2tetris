...
class CodeWriter:
    ...
    def eq_gt_lt(self, command):
        ...
        # handle overflow
            ...
            self._output.write(
                f"@ISNOTEQ_{self._filename.upper()}_{self._label_num}\n")
        ...
        self._output.write(
            f"@TRUE_{self._filename.upper()}_{self._label_num}\n")
        self._output.write(f"D;{asm_condition}\n")
        self._output.write(f"@{self.STACK_POINTER}\n")
        self._output.write("A=M\n")
        self._output.write("M=0\n")
        self._output.write(
            f"@ENDIF_{self._filename.upper()}_{self._label_num}\n")
    ...
    def write_init(self):
        self._output.write("@SP\n")
        self._output.write("M=0\n")
        self._output.write(f"@{self.STACK_START_ADDRESS}\n")
        self._output.write("D=A\n")
        self._output.write("@SP\n")
        self._output.write("M=D\n")

        for i in range(5):
            self._output.write("D=0\n")
            self.stack_push()
            self.increment_sp()
        self._output.write(f"@{self.BOOTSTRAP_CALL}\n")
        self._output.write("0;JMP\n")
    ...
    def write_function(self, f_name, n_vars):
        self._output.write(f"({f_name})\n")
        self._output.write(f"@{n_vars}\n")
        self._output.write("D=A\n")
        # (LOOP)
        self._output.write(
            f"(LOOP_{self._filename.upper()}_{self._label_num})\n")
        # if (n_vars == 0) jump to (ENDFUNCTION)
        self._output.write(
            f"@ENDFUNCTION_{self._filename.upper()}_{self._label_num}\n")
        self._output.write("D;JEQ\n")
        # else: push 0
        self._output.write(f"@{self.STACK_POINTER}\n")
        self._output.write("A=M\n")
        self._output.write("M=0\n")
        self.increment_sp()
        self._output.write("D=D-1\n")
        self._output.write(
            f"@LOOP_{self._filename.upper()}_{self._label_num}\n")
        self._output.write("0;JMP\n")
        # (ENDFUNCTION)
    ...
    def write_call(self, f_name, n_args):
        # push return_address
        self._output.write(
            f"@RETURNADDRESS_{self._filename.upper()}_{self._label_num}\n")
        self._output.write("D=A\n")
        self.stack_push()
        self.increment_sp()
        # push LCL, ARG, THIS and THAT
        for vm_segment in self.VM_SEGMENTS[:4]:
            self._output.write(f"@{self.ASM_SEGMENTS[vm_segment]}\n")
            self._output.write("D=M\n")
            self.stack_push()
            self.increment_sp()
        # update ARG's address
        self._output.write("@SP\n")
        self._output.write("D=M\n")
        self._output.write(f"@{self.SAVED_ADDRESSES_NUM}\n")
        self._output.write("D=D-A\n")
        self._output.write(f"@{n_args}\n")
        self._output.write("D=D-A\n")
        self._output.write("@ARG\n")
        self._output.write("M=D\n")
        # update LCL
        self._output.write("@SP\n")
        self._output.write("D=M\n")
        self._output.write("@LCL\n")
        self._output.write("M=D\n")
        # goto function name
    ...
    def write_return(self):
        ...
        # SP = ARG + 1
        self._output.write("@ARG\n")
        self._output.write("D=M+1\n")
        self._output.write("@SP\n")
        self._output.write("M=D\n")
        # THAT/THIS/ARG/LCL = *(LCL - i)
        for i in (3, 2, 1, 0):
            segment = self.ASM_SEGMENTS[self.VM_SEGMENTS[i]]
            self._output.write("@R13\n")
            self._output.write("AM=M-1\n")
            self._output.write("D=M\n")
            self._output.write(f"@{segment}\n")
            self._output.write("M=D\n")
        # goto return address
        self._output.write("@R14\n")
        self._output.write("A=M\n")
